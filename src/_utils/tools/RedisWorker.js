...
export default class RedisWorker {
    static client = createClient(clientSettings);

    name = ""
    type
    handleRequest = async ({ key, value, command, attemptingType, ex }) => {
        try {
        ...
        if (!type) {
            ...
            if (ex) await RedisWorker.client.expire()
        ...
        if (type === attemptingType) {
            ...
            if (ex) await RedisWorker.client.expire(args[0])
            return response

        }
        else console.error(`This key is storing the "${type}" data type \nType received: "${attemptingType}"`)
    }
    ...
    async hSet(key, value, ex) {
        if (typeof value === "object") return await this.handleRequest({
    ...
    async hGet(key, fields) {
        if (typeof fields === "string" || Array.isArray(fields)) return await this.handleRequest({
            attemptingType: "object",
            command: "hGet",
            key,
            value: fields
        })
        else throw new Error(`Didn't get expected fields, got typeof: ${typeof fields}`)
    }
    ...
    async connect() {
        if (!RedisWorker.client.isOpen) await RedisWorker.client.connect()
    }
